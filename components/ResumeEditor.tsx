...
const ResumeEditor: React.FC<ResumeEditorProps> = ({ data, onChange }) => {
  ...
  const generateLaTeX = () => {
    const { contact, summary, education, experience, projects, skillCategories } = data;
    let tex = `\\documentclass[a4paper,10pt]{article}\n\\usepackage[utf8]{inputenc}\n\\usepackage{hyperref}\n\\begin{document}\n\n`;
    tex += `\\centerline{\\Large \\textbf{${contact.fullName}}}\n`;
    tex += `\\centerline{${contact.professionalTitle} | ${contact.email} | ${contact.phone}}\n\\hr\n\n`;
    
    tex += `\\section*{Summary}\n${summary}\n\n`;
    
    tex += `\\section*{Education}\n`;
    education.forEach(edu => {
      tex += `\\textbf{${edu.school}} \\hfill ${edu.startDate} - ${edu.endDate}\\\\ \n${edu.degree} in ${edu.specialization} | Result: ${edu.result}\\\\ \n\n`;
    });

    tex += `\\section*{Experience}\n`;
    experience.forEach(exp => {
      tex += `\\textbf{${exp.company}} \\hfill ${exp.startDate} - ${exp.endDate}\\\\ \n\\textit{${exp.role}}\\\\ \n\\begin{itemize}\n`;
      exp.description.forEach(point => tex += `  \\item ${point}\n`);
      tex += `\\end{itemize}\n\n`;
    });
    ...
    <div className="flex flex-col h-full bg-[#0f172a] overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-8 scroll-smooth scrollbar-thin scrollbar-thumb-slate-700">
        {activeTab === EditorTab.CONTACT && (
          <div className="space-y-6 animate-in fade-in duration-300">
            <div className="flex items-center justify-between p-3 bg-slate-900 border border-slate-800 rounded-lg">
              <span className="text-xs font-black uppercase text-slate-500">Link Mode: {data.contact.showFullLinks ? 'FULL URL' : 'LABEL'}</span>
              <button 
        ...
        {activeTab === EditorTab.ORDER && (
          <div className="space-y-4">
            <label className={labelClasses}>Hierarchy Reordering</label>
        ...
        {activeTab === EditorTab.SUMMARY && (
           <div className="space-y-4">
             <label className={labelClasses}>Objective Summary</label>
             <textarea rows={12} className={inputClasses} value={data.summary || ''} onChange={(e) => onChange({...data, summary: e.target.value})} placeholder="Professional summary..." />
           </div>
        )}

        {activeTab === EditorTab.SKILLS && (
          <div className="space-y-6">
            <button onClick={() => addItem('skillCategories', { name: '', skills: [] })} className="w-full py-2 bg-blue-600/10 border border-blue-500/20 text-[10px] font-black uppercase text-blue-400 rounded-lg">+ Add Skill Group</button>
            {data.skillCategories.map(cat => (
              <div key={cat.id} className="p-5 bg-slate-900 border border-slate-800 rounded-xl space-y-4 relative">
                ...
                <input type="text" placeholder="Category Name (e.g. Frontend)" className={inputClasses} value={cat.name || ''} onChange={(e) => updateItem<SkillCategory>('skillCategories', cat.id, { name: e.target.value })} />
        ...
        {activeTab === EditorTab.WORK && (
          <div className="space-y-6">
            <button onClick={() => addItem('experience', { company: '', role: '', startDate: '', endDate: '', description: [''] })} className="w-full py-2 bg-blue-600/10 border border-blue-500/20 text-[10px] font-black uppercase text-blue-400 rounded-lg">+ Add Experience</button>
            {data.experience.map(exp => (
              <div key={exp.id} className="p-5 bg-slate-900 border border-slate-800 rounded-xl space-y-4 relative">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <input type="text" placeholder="Start" className={inputClasses} value={exp.startDate || ''} onChange={(e) => updateItem<Experience>('experience', exp.id, { startDate: e.target.value })} />
                  <input type="text" placeholder="End" className={inputClasses} value={exp.endDate || ''} onChange={(e) => updateItem<Experience>('experience', exp.id, { endDate: e.target.value })} />
                </div>
                <textarea className={inputClasses} rows={5} placeholder="Achievements (one per line)" value={(exp.description || []).join('\n')} onChange={(e) => updateItem<Experience>('experience', exp.id, { description: e.target.value.split('\n') })} />
              </div>
            ))}
          </div>
        )}
        ...
        {activeTab === EditorTab.PROJECTS && (
          <div className="space-y-6">
            <button onClick={() => addItem('projects', { name: '', organization: '', date: '', description: [''] })} className="w-full py-2 bg-blue-600/10 border border-blue-500/20 text-[10px] font-black uppercase text-blue-400 rounded-lg">+ Add Project</button>
            {data.projects.map(proj => (
              <div key={proj.id} className="p-5 bg-slate-900 border border-slate-800 rounded-xl space-y-4 relative">
                ...
                <input type="text" placeholder="Organization / Prof." className={inputClasses} value={proj.organization || ''} onChange={(e) => updateItem<Project>('projects', proj.id, { organization: e.target.value })} />
                ...
                <textarea className={inputClasses} rows={4} placeholder="Details (one per line)" value={(proj.description || []).join('\n')} onChange={(e) => updateItem<Project>('projects', proj.id, { description: e.target.value.split('\n') })} />
              </div>
            ))}
          </div>
        )}
        ...
        {activeTab === EditorTab.EDUCATION && (
          <div className="space-y-6">
            <button onClick={() => addItem('education', { school: '', degree: '', specialization: '', result: '', startDate: '', endDate: '' })} className="w-full py-2 bg-blue-600/10 border border-blue-500/20 text-[10px] font-black uppercase text-blue-400 rounded-lg">+ Add Education</button>
            {data.education.map(edu => (
              <div key={edu.id} className="p-5 bg-slate-900 border border-slate-800 rounded-xl space-y-4 relative">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <input type="text" placeholder="Degree" className={inputClasses} value={edu.degree || ''} onChange={(e) => updateItem<Education>('education', edu.id, { degree: e.target.value })} />
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <input type="text" placeholder="CPI / %" className={inputClasses} value={edu.result || ''} onChange={(e) => updateItem<Education>('education', edu.id, { result: e.target.value })} />
                </div>
              </div>
            ))}
          </div>
        ...
        {activeTab === EditorTab.CUSTOM && (
          <div className="space-y-6">
            <button onClick={() => addItem('customSections', { title: '', content: [''] })} className="w-full py-2 bg-blue-600/10 border border-blue-500/20 text-[10px] font-black uppercase text-blue-400 rounded-lg">+ Add Section Block</button>
            {data.customSections.map(sec => (
              <div key={sec.id} className="p-5 bg-slate-900 border border-slate-800 rounded-xl space-y-4 relative">
                ...
                <input type="text" placeholder="Header" className={inputClasses} value={sec.title || ''} onChange={(e) => updateItem<CustomSection>('customSections', sec.id, { title: e.target.value })} />
                <textarea className={inputClasses} rows={6} placeholder="Bullets (one per line)" value={(sec.content || []).join('\n')} onChange={(e) => updateItem<CustomSection>('customSections', sec.id, { content: e.target.value.split('\n') })} />
              </div>
            ))}
          </div>
        )}
        ...
        {activeTab === EditorTab.DESIGN && (
          <div className="space-y-4">
            <label className={labelClasses}>Custom CSS Overrides</label>
            <textarea rows={15} className="w-full bg-slate-950 border border-slate-700 rounded-lg p-4 font-mono text-xs text-blue-400 outline-none" value={data.customCSS || ''} onChange={(e) => onChange({...data, customCSS: e.target.value})} placeholder=".section-header { ... }" />
          </div>
        )}
        ...
        {activeTab === EditorTab.LATEX && (
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              <label className={labelClasses}>LaTeX Source (Draft)</label>
              <button 
                onClick={() => downloadFile(generateLaTeX(), 'resume.tex', 'text/plain')}
                className="px-4 py-2 bg-blue-600 text-white rounded-lg text-[10px] font-black uppercase"
              >
                Download .tex
              </button>
            </div>
            <textarea 
              rows={20} 
              readOnly
              className="w-full bg-slate-950 border border-slate-700 rounded-lg p-4 font-mono text-[10px] text-slate-400 outline-none" 
              value={generateLaTeX()} 
            />
          </div>
        )}
        ...
        {activeTab === EditorTab.SYSTEM && (
          <div className="h-full flex flex-col">
            <label className={labelClasses}>System JSON State</label>
            <div className="flex-1 min-h-[500px] border border-slate-800 rounded-xl overflow-hidden">
